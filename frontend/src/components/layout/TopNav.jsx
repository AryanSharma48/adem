...
import { Bell, CloudSun, Menu, Lightbulb, MapPin } from 'lucide-react';
  ...
  return (
    <div className="bg-white border-b border-slate-200 h-16 md:h-20 px-3 md:px-8 flex items-center justify-between">
      <div className="flex items-center min-w-0">
        ...
        <div className="min-w-0 flex flex-col justify-center">
          <h2 className="text-base sm:text-lg md:text-2xl font-bold text-slate-900 flex items-center whitespace-nowrap truncate">
            <span className="md:hidden tracking-tight mr-1">AD<span className="text-blue-600">EM</span></span>
            ...
            <div className="ml-2 md:ml-3 flex items-center text-[10px] md:text-sm font-medium text-slate-600 bg-slate-100 px-2 py-0.5 md:px-2.5 md:py-1 rounded-full shrink-0">
              <MapPin className="w-3 h-3 md:w-4 md:h-4 mr-0.5 md:mr-1 text-red-500" />
              <span className="mr-1.5 md:mr-2">Astana</span>
      ...
      <div className="flex items-center space-x-3 md:space-x-6 shrink-0">
        ...
        <div 
          className="flex items-center bg-slate-100 rounded-full p-0.5 md:p-1 cursor-pointer hover:bg-slate-200 transition-colors shrink-0"
          onClick={toggleLanguage}
          aria-label="Toggle language"
        >
          <div className={`px-2 md:px-2.5 py-0.5 md:py-1 text-[10px] md:text-xs font-bold rounded-full transition-all ${i18n.language === 'en' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
            EN
          </div>
          <div className={`px-2 md:px-2.5 py-0.5 md:py-1 text-[10px] md:text-xs font-bold rounded-full transition-all ${i18n.language === 'kk' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
            KZ
          </div>
        </div>
        ...
        <div className="text-right">
          <p className="block text-[8px] md:text-xs text-slate-500 uppercase font-bold tracking-wider">Updated at</p>
          {loading ? (
